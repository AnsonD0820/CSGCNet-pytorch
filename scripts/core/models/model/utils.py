...
###图推理块
    def __init__(self, num_in, num_mid, stride=(1,1), kernel=1):
        ...
        # generate projection and inverse projection functions 生成投影和反投影函数
        ...
        # reasoning by graph convolution  图卷积推理
    ...
    def forward(self, x):
        batch_size = x.size(0)    ##  x = x.view(x.size(0), -1)  这句话的出现就是为了将前面多维度的tensor展平成一维
        # generate projection and inverse projection matrices 生成投影和逆投影矩阵
        x_state_reshaped = self.conv_state(x).view(batch_size, self.num_s, -1) #降维
        x_proj_reshaped = self.conv_proj(x).view(batch_size, self.num_n, -1)  #投影函数
        x_rproj_reshaped = self.conv_reproj(x).view(batch_size, self.num_n, -1)  #反投影函数
        # project to node space  项目到节点空间
        x_n_state1 = torch.bmm(x_state_reshaped, x_proj_reshaped.permute(0, 2, 1))  #执行矩阵乘积
        ...
        # inverse project to original space    反投影矩阵和节点特征执行矩阵相乘重溯到原始空间
